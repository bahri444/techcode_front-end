<template>
  <div>
    <!-- student_class_uuid	user_uuid	class_uuid	date_checkout_class	price_state -->
    <NuxtLayout name="sidebar"
      ><div class="container">
        <p class="font-semibold text-center mt-2 mb-5">Students class</p>
        <NuxtLink
          to="/admin/student_class/addclass_student"
        ...
        <table class="w-full text-sm text-left text-slate-500">
          <thead
            ...
            <tr>
              <th scope="col" class="px-6 py-4">studen class uuid</th>
              ...
              <th scope="col" class="px-6 py-4">date checkout class</th>
              <th scope="col" class="px-6 py-4">price state</th>
          ...
          <tbody>
            ...
            <tr
              v-for="vals in student_class"
              :key="vals.class_uuid"
              class="bg-white border-b hover:bg-slate-100 text-center">
              <td class="px-6 py-4">{{ vals.student_class_uuid }}</td>
              <td class="px-6 py-4">{{ vals.user_uuid }}</td>
              <td class="px-6 py-4">{{ vals.nama_lengkap }}</td>
              ...
              <td class="px-6 py-4">{{ vals.date_checkout_class }}</td>
              <td class="px-6 py-4">{{ vals.price_state }}</td>
              ...
              <td class="text-center px-6 py-4">
                <div class="grid lg:grid-cols-2 md:grid-cols-1 w-44 mx-auto">
                  <!-- BUTTON EDIT -->
                  <NuxtLink
                    :to="'/admin/student_class/' + vals.student_class_uuid"
...
<script>
  ...
  export default {
    ...
    methods: {
      GetAllStudentClass() {
        axios.get(BaseUrl + "/api/getallstudentclass").then((res) => {
