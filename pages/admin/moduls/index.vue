      ...
      <div class="container">
        ...
        <table class="w-full text-sm text-left text-slate-500">
          <thead
            ...
            <tr>
              <th scope="col" class="px-6 py-4">e-book categories uuid</th>
              <th scope="col" class="px-6 py-4">e-book categories name</th>
              <th scope="col" class="px-6 py-4">class uuid</th>
              <th scope="col" class="px-6 py-4">class name</th>
              <th scope="col" class="px-6 py-4">modul title</th>
              <th scope="col" class="px-6 py-4">modul files</th>
              <th scope="col" class="px-6 py-4">modul to</th>
              <th scope="col" class="px-6 py-4">learn state</th>
              <th scope="col" class="px-6 py-4">Aksi</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="vals in data_moduls"
              :key="vals.modul_uuid"
              class="bg-white border-b hover:bg-slate-100 text-center">
              <td class="px-6 py-4">{{ vals.modul_categories_uuid }}</td>
              <td
                class="px-6 py-4"
                v-for="modul_categories in vals.join_to_modul_categories">
                {{ modul_categories.categories_name }}
              </td>
              <td class="px-6 py-4">{{ vals.class_uuid }}</td>
              <td class="px-6 py-4" v-for="classes in vals.join_to_class">
                {{ classes.class_name }}
              </td>
              <td class="px-6 py-4">{{ vals.modul_title }}</td>
              <td class="px-6 py-4">{{ vals.modul_files }}</td>
              <td class="px-6 py-4">{{ vals.modul_to }}</td>
              <td class="px-6 py-4">{{ vals.learn_state }}</td>
              <td class="text-center px-6 py-4">
                <div class="grid lg:grid-cols-3 md:grid-cols-1 w-44 mx-auto">
                  ...
                  <!-- BUTTON INFO -->
                  <NuxtLink
                    :to="'/admin/moduls/' + vals.modul_uuid"
