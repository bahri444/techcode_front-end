      ...
      <div class="container">
        ...
        <table class="w-full text-sm text-left text-slate-500">
          <thead
            ...
            <tr>
              <th scope="col" class="px-6 py-4">Kategories uuid</th>
              <th scope="col" class="px-6 py-4">Actifity uuid</th>
              <th scope="col" class="px-6 py-4">Actifity categories name</th>
              <th scope="col" class="px-6 py-4">Actifity name</th>
              <th scope="col" class="px-6 py-4">Place actifity</th>
              <th scope="col" class="px-6 py-4">Start date</th>
              <th scope="col" class="px-6 py-4">End time</th>
              <th scope="col" class="px-6 py-4">Actifity status</th>
              <th scope="col" class="px-6 py-4">Aksi</th>
            </tr>
          </thead>
          <tbody>
            <tr
              v-for="rows in data_actifity"
              :key="rows.actifity_uuid"
              class="bg-white border-b hover:bg-slate-100 text-center">
              <td class="px-6 py-4">{{ rows.actifity_categories_uuid }}</td>
              <td class="px-6 py-4">{{ rows.actifity_uuid }}</td>
              <td
                class="text-center px-6 py-4"
                v-for="newrows in rows.to_categories_actifity">
                {{ newrows.actifity_categories_name }}
              </td>
              <!-- {{
                rows[0]["to_categories_actifity"]["actifity_categories_uuid"]
              }} -->
              <td class="px-6 py-4">{{ rows.actifity_name }}</td>
              <!-- <td>{{ rows.description }}</td> -->
              <td class="text-center px-6 py-4">{{ rows.place_actifity }}</td>
              <td class="text-center px-6 py-4">{{ rows.start_date }}</td>
              <td class="text-center px-6 py-4">{{ rows.end_date }}</td>
              <td class="text-center px-6 py-4">{{ rows.actifity_status }}</td>
              <td class="text-center px-6 py-4">
                <div class="grid lg:grid-cols-3 md:grid-cols-1 w-44 mx-auto">
                  ...
                  <NuxtLink
                    ...
                    class="py-2 w-12 bg-teal-500 text-white font-semibold rounded-lg shadow-md hover:bg-teal-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 ml-1"
                    ><svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="#f1f5f9"
                      ...
                      <g data-name="Layer 2">
                        <g data-name="info">
                          <path
                            d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm0 18a8 8 0 1 1 8-8 8 8 0 0 1-8 8z"></path>
                          <circle cx="12" cy="8" r="1"></circle>
                          <path
                            d="M12 10a1 1 0 0 0-1 1v5a1 1 0 0 0 2 0v-5a1 1 0 0 0-1-1z"></path>
                        </g>
                      </g>
                    </svg>
                  </NuxtLink>
